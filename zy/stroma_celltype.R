...
seurat.stroma <- FindClusters(seurat.stroma, resolution = 0.2)
DimPlot(seurat.stroma, group.by = "RNA_snn_res.0.2", label = T, reduction = 'umap', pt.size = 1)
...
cluster_0.3 <- seurat.stroma$RNA_snn_res.0.2
...
ggsave(plot.umap, path = path.lineage, 
       filename = 'stroma_lineage_umap.png',
       height = 10, width = 15, units = 'cm')


plot.cluster <-
    DimPlot(seurat.stroma, reduction = 'umap', 
            group.by = "RNA_snn_res.0.2", label = T,
            pt.size = 0.8, label.size = 5.5) + 
    # ylim(-12, 15) + 
    labs(x = 'UMAP1', y = 'UMAP2') +
    theme(panel.background=element_rect(fill='transparent', color='black',size = 1),
...
ggsave(plot.cluster, path = path.lineage, 
       filename = 'stroma_lineage_umap_cluster.png',
       height = 10, width = 15, units = 'cm')
